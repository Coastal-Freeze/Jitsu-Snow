from snow.data import AbstractDataCollection, db
...
class Card(db.Model):
    ...
    element = db.Column(
        db.CHAR(1), nullable=False, server_default=db.text("'s'::bpchar")
    )
...
class CardStarterDeck(db.Model):
    ...
    card_id = db.Column(
        db.ForeignKey("card.id", ondelete="CASCADE", onupdate="CASCADE"),
        primary_key=True,
        nullable=False,
    )
