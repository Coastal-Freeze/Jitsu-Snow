...
class Penguin(Client, penguin.Penguin):
    ...
    async def show_ui(self):
        await self.send_json(action='loadWindow', assetPath='', initializationPayload={
            ...
            'element': self.ninja.element.value, 'isMember': self.is_member}, layerName='bottomLayer',
                             loadDescription='', type='playAction',
    ...
    async def show_round_notice(self, round_num, bonus_criteria, remaining_time=0):
        await self.send_json(action='loadWindow', assetPath='',
                             initializationPayload={'bonusCriteria': bonus_criteria, 'roundNumber': round_num,
