...
class SnowMatchMaking:
    ...
    async def start(self):
        while True:
            ...
            await asyncio.sleep(
                1
            )  # blocks whole thing allow other coroutines to finish

    async def match_queue(self):
        while all([len(players) > 0 for players in self._penguins.values()]):
            match_players = [
                self._penguins["fire"].pop(0),
                self._penguins["water"].pop(0),
            ...
            for i, penguin in enumerate(match_players):
                ...
                await penguin.send_tag(
                    "W_PLACELIST",
                    "0:10001",
                    "snow_1",
                    "3 player sex scenario",
                    1,
                    9,
                    5,
                    0,
                ...
                await penguin.send_json(
                    action="jsonPayload",
                    jsonPayload={
                        "1": match_players[0].safe_name,
                        "2": match_players[1].safe_name,
