...
class AnimationManager:
    ...
    async def display_target(self, player, target_obj):
        await player.send_tag('O_SPRITE', target_obj.id, target_obj.parent.sprite_animation.value, 1)
        await player.send_tag('O_SPRITEANIM', target_obj.id, 1, 6, 1, 'play_once',
                              target_obj.parent.SpriteDuration.value)

        await asyncio.sleep(target_obj.parent.SpriteDuration.value * 0.001)

        await player.send_tag('O_SPRITE', target_obj.id, target_obj.parent.sprite_loop_animation.value, 1)
        await player.send_tag('O_SPRITEANIM', target_obj.id, 1, 60, 0, 'loop',
                              target_obj.parent.SpriteLoopDuration.value)

    async def green_target(self, player, target_obj):
        await player.send_tag('O_SPRITE', target_obj.id, target_obj.parent.sprite_animation.value, 1)
        await player.send_tag('O_SPRITEANIM', target_obj.id, 1, 6, 0, 'play_once',
                              target_obj.parent.SpriteDuration.value)
