...
class ObjectManager:
    ...
    def generate_map(self):
        for column in range(MapConstants.board_width.value):
            ...
            for row in range(MapConstants.board_height.value):
                tile_object = self.generate_object(
                    MovementTile.art_index.value,
                    MovementTile.template_id.value,
                    column,
                    row,
                    parent=MovementTile,
                )
    ...
    def generate_players(self):
        ...
        for i, ninja in enumerate(ninjas):
            y_coordinate = 2 * i
            self.generate_player_object(
                MapConstants.player_object_ids.value[i], ninja, 0, y_coordinate
            )
    ...
    def generate_enemies(self):
        ...
        for i in range(count):
            ...
            self.generate_enemy_hp_object(
                x, y, enemy_type=self.room.enemy_manager.default_enemies[0]
            )

            random.shuffle(self.room.enemy_manager.default_enemies)

    def generate_check_mark(self, ninja_type, x, y):
        check_mark = self.generate_object("0:1", "0:30049", x, y, parent=ninja_type)

        self.check_marks.append(check_mark)
        return check_mark

    def generate_damage_particle(self, ninja_type, x, y):
        damage_number = self.generate_object("0:1", "0:30059", x, y, parent=ninja_type)
        tile_particle = self.generate_object("0:1", "0:30059", x, y, parent=ninja_type)
    ...
    def generate_enemy_object(self, enemy_type, x, y):
        enemy_object = self.generate_object(
            EnemyObject.art_index.value,
            EnemyObject.template_id.value,
            x,
            y,
            parent=EnemyObject,
            owner=enemy_type,
    ...
    def generate_enemy_hp_object(self, x, y, enemy_type=None):
        enemy_hp_object = self.generate_object(
            HPObject.art_index.value,
            HPObject.template_id.value,
            x,
            y,
            parent=HPObject,
            owner=enemy_type,
    ...
    def generate_obstacle_object(self, x, y):
        obstacle_object = self.generate_object(
            ObstacleObject.art_index.value,
            ObstacleObject.template_id.value,
            x,
            y,
            parent=ObstacleObject,
        )
    ...
    def generate_player_hp_object(self, x, y, ninja_type=None):
        player_hp_object = self.generate_object(
            HPObject.art_index.value,
            HPObject.template_id.value,
            x,
            y,
    ...
    def generate_player_ghost(self, ninja_type, x, y):
        ghost_object = self.generate_object("0:1", "0:30017", x, y, parent=ninja_type)

        self.temp_objects.append(ghost_object)
        return ghost_object

    def generate_object(
        self, art_index, template_id, x, y, name=None, parent=None, owner=None
    ):
        if name is None:
    ...
    async def check_mark(self, p):
        ...
        adjusted_x = round(
            check_object.x + PenguinObject.x_coordinate_offset.value,
            PenguinObject.x_coordinate_decimals.value,
        )
        adjusted_y = round(
            check_object.y + PenguinObject.y_coordinate_offset.value,
            PenguinObject.y_coordinate_decimals.value,
        )
        await p.room.send_tag(
            "O_HERE",
            check_object.id,
            "0:1",
            adjusted_x,
    ...
    async def plan_movement(self, p, tile):
        ...
        adjusted_x = round(
            ghost_object.x + PenguinObject.x_coordinate_offset.value,
            PenguinObject.x_coordinate_decimals.value,
        )
        adjusted_y = round(
            ghost_object.y + PenguinObject.y_coordinate_offset.value,
            PenguinObject.y_coordinate_decimals.value,
        )
        ...
        await p.room.send_tag(
            "O_SPRITE", ghost_object.id, p.ninja.selected_tile_animation.value, 1, ""
        )
        await p.room.sound_manager.play_sound("0:1840040")
    ...
    async def show_targets(self, p, x, y):
        ...
        potential_tiles = self.get_tiles_from_range(
            x, y, tile_range=p.ninja.range.value
        )
        for tile in potential_tiles:
            await self.generate_heal_tiles(p, tile)
            if tile.owner in self.room.enemy_manager.default_enemies:
                target_obj = self.generate_object(
                    EnemyTarget.art_index.value,
                    EnemyTarget.template_id.value,
                    tile.x,
                    tile.y,
                    parent=EnemyTarget,
                )
                ...
                adjusted_x = round(
                    tile.x + EnemyTarget.x_coordinate_offset.value,
                    EnemyTarget.x_coordinate_decimals.value,
                )
                adjusted_y = round(
                    tile.y + EnemyTarget.y_coordinate_offset.value,
                ...
                await p.send_tag(
                    ...
                    0,
                    target_obj.name,
                    target_obj.template_id,
                    0,
                    0,
                    0,
                )
                asyncio.create_task(
                    self.room.animation_manager.display_target(p, target_obj)
                )
                await p.room.sound_manager.play_individual_sound(
                    p, target_obj.parent.sound.value
                )
    ...
    async def generate_heal_tiles(self, p, tile):
        if p.ninja == SnowNinja and tile.owner in [
            FireNinja,
            WaterNinja,
        ]:  # Healing Target
            target_obj = self.generate_object(
                PenguinTarget.art_index.value,
                PenguinTarget.template_id.value,
                tile.x,
                tile.y,
                parent=PenguinTarget,
            )

            adjusted_x = round(
                tile.x + PenguinTarget.x_coordinate_offset.value,
                PenguinTarget.x_coordinate_decimals.value,
            )
            adjusted_y = round(
                tile.y + PenguinTarget.y_coordinate_offset.value,
    ...
    async def select_enemy(self, p, enemy_id):
        ...
        for target_obj in p.target_objects:
            if target_obj.x == enemy.x and target_obj.y == enemy.y:
                ...
                asyncio.create_task(
                    self.room.animation_manager.green_target(p, target_obj)
                )
            else:
    ...
    async def player_move(self, penguin):
        ...
        adjusted_x = round(
            new_tile.x + PenguinObject.x_coordinate_offset.value,
            PenguinObject.x_coordinate_decimals.value,
        )
        adjusted_y = round(
            new_tile.y + PenguinObject.y_coordinate_offset.value,
            PenguinObject.y_coordinate_decimals.value,
        )
        ...
        await self.room.sound_manager.play_sound(
            penguin.ninja.move_animation_sound.value
        )

        adjusted_x = round(
            healthbar.x + HPObject.x_coordinate_offset.value,
            HPObject.x_coordinate_decimals.value,
        )
        adjusted_y = round(
            healthbar.y + HPObject.y_coordinate_offset.value,
    ...
    async def do_move_action(self):
        for penguin in self.get_alive_ninjas():
            ...
            if penguin.current_target is not None:
                await self.room.player_manager.player_damage(
                    penguin, penguin.current_target, penguin.ninja.attack.value
                )
    ...
    async def send_map(self, p):
        for column in self.map:
            for tile in column:
                adjusted_x = round(
                    tile.x + tile.parent.x_coordinate_offset.value,
                    tile.parent.x_coordinate_decimals.value,
                )
                adjusted_y = round(
                    tile.y + tile.parent.y_coordinate_offset.value,
        ...
        for obj in self.obstacles:
            adjusted_x = round(
                obj.x + obj.parent.x_coordinate_offset.value,
                obj.parent.x_coordinate_decimals.value,
            )
            adjusted_y = round(
                obj.y + obj.parent.y_coordinate_offset.value,
                obj.parent.y_coordinate_decimals.value,
            )
        ...
        for i, player_obj in enumerate(self.players):
            ...
            adjusted_x = round(
                player_obj.x + player_obj.parent.x_coordinate_offset.value,
                player_obj.parent.x_coordinate_decimals.value,
            )
            adjusted_y = round(
                player_obj.y + player_obj.parent.y_coordinate_offset.value,
            ...
            adjusted_x = round(
                player_hp_obj.x + player_hp_obj.parent.x_coordinate_offset.value,
                player_hp_obj.parent.x_coordinate_decimals.value,
            )
            adjusted_y = round(
                player_hp_obj.y + player_hp_obj.parent.y_coordinate_offset.value,
