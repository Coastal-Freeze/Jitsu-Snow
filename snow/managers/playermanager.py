...
class PlayerManager:
    ...
    async def player_death(self, p):
        await self.animation_manager.play_animation(
            p.tile, p.ninja.knockout_animation.value, "play_once", 2500
        )
        await asyncio.sleep(2.5)
        await self.animation_manager.play_animation(
            p.tile, p.ninja.knockout_animation_loop.value, "loop", 800
        )
        p.damage = p.ninja.health_points.value
        hp_bar = self.object_manager.player_hpbars[
    ...
    async def player_heal(self, p):
        ...
        await self.animation_manager.play_animation(
            penguin.tile,
            penguin.ninja.heal_animation.value,
            "play_once",
    ...
    async def player_healing(self, penguin, target):
        await penguin.room.sound_manager.play_sound(
            penguin.ninja.attack_animation_sound.value
        )
        hp_bar = self.object_manager.player_hpbars[
            self.object_manager.players.index(target)
        ]
        penguin.damage -= 12
        hp_percentage = round(
            (penguin.damage * 100) / penguin.ninja.health_points.value
        )
    ...
    async def player_damage(self, p, enemy, damage, player_animation = True):
        ...
        await self.animation_manager.play_animation(
            enemy,
            enemy.owner.hit_animation.value,
            "play_once",
