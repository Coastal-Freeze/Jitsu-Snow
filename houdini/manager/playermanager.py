...
class PlayerManager:    
    ...
    async def player_death(self, p):
        ...
        hpbar = self.object_manager.player_hpbars[self.players.index((n_obj.current_object))]
    ...
    async def player_heal(self, p):
        target = player.heal_target
        penguin = self.get_penguin_by_ninja_type(target.parent)
    ...
    async def player_healing(self, penguin, target):
        ...
        hpbar = self.player_hpbars[self.players.index(target)]
